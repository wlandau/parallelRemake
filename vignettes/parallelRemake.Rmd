...
3. Call `makefile()` to generate the `Makefile` and run it. Use `command = "make -j 4"` to distribute the work over 4 parallel processes.
...
```r
makefile(.., command = "make -j 8", prepend = "SHELL = ./shell.sh")
```
